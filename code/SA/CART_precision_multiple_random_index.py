...
# Form the neighbour using multiple random indices and random values selection
def neighbour_selection(parameters, old_tunings):

    dic_length = len(parameters)

    # Form the list of random indices
    random_index = random.sample(range(0, dic_length), 3)

    # print random_index

    random_index_tuning_value = []

    for i in random_index:
        x = np.random.uniform(parameters[i]['low'], parameters[i]['high'])

        if i != 0:
            x = int(x)

        random_index_tuning_value.append(x)
...
def simulated_annealing_random(dataset):
    ...
    while T >= T_min:
        ...
        while i <= 50:
            ...
            print new_tuning_index

            print new_tuning_index_value

            for j in range(0, len(new_tuning_index)):
                new_tunings[new_tuning_index[j]] = new_tuning_index_value[j]
